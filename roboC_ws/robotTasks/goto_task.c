...
int goto_task(float x, float y, float yaw, float left_sensor_dist, float right_sensor_dist, float mid_sensor_dist, int opp_detected, float ball_yaw){
    ...
    if(ball_location == FRONT && opp_detected){ //Opponent, not ball
        //stop movement
        goto_linX = 0.0;
        goto_angZ = 0.0;
    ...
    else if(ball_location == FRONT && !opp_detected){ //Ball in front, check if need to move forward to get closer to the ball
