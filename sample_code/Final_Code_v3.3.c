	...
	for (int count_deg = 1; count_deg < 80; count_deg++){
		...
		//BALL FOUND WHILE ROTATING LEFT BY THE LEFT SENSOR
		  ...
		  //if right sensor detect, get the current time and distance to the ball by right sensor, and rotate to the centre
                ...
                turn_right_infinite();
                wait1Msec(time_to_the_centre+4000);
                stop_motors();
                time_clear = 0;
			}
			state = 2;
		...
		if (right_sensor() == 1){	//If the ball is not found, keep rotating until 90 deg
			...
			if (left_sensor() == 1){ //Turn right until left sensor detect
		     ...
		     //if right sensor detect, get the current time and distance to the ball by right sensor, and rotate to the centre
				...
				float time_to_the_centre = ((time_1)/2);
      	        turn_left_infinite();
             	wait1Msec(time_to_the_centre+4000);  //Go the the centre
             	stop_motors();
            	time_clear = 0;
			}
            state = 2;
...
void return_state(){
    ...
    if (check_boundary() != 0){
		...
		if ((SensorValue(switch_left)==1)&&(SensorValue(switch_right)==1)){
			...
			move_backward(25);
			if ((SensorValue(switch_left)==1)&&(SensorValue(switch_right)==0)){
				if (start_T3 == 0){
					clearTimer(T3);
					time_3 = time1[T3];
					start_T3 = 1;
			...
			else if ((SensorValue(switch_left)==0)&&(SensorValue(switch_right)==1)){
					if (start_T3 == 0){
						clearTimer(T3);
						time_3 = time1[T3];
						start_T3 = 1;
